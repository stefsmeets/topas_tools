...
def parse_str_int_float(item):
	...
	try:
		...
		return item
#	except TypeError:
#		print 'TypeError in parsing {}, because it has type {}:'.format(item, type(item))
#	else:
#		return eval(item)
...
def get_lines(lines):
	...
	for line in lines:
		...
		if not line:
			...
			yield line.split("'")[0].replace(')','').split()	# split on ' and return list of rest, except for the number
...
def plot_2d(iterator,x_key,y_key,picker=['spgr','num']):
	...
	#col = ax.scatter(x, y, 100*s, c, picker=True) => s,c add color/size to points, maybe nice for best solutions from sflog ??
